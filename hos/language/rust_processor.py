...
class RustProcessor(LanguageProcessor):
    ...
    def analyze(self, code):
        ...
        analysis_result = {
            'ast': {
                'functions': functions,
                'classes': structs + enums,  # 将struct和enum视为类
                'imports': imports
            },
            'cfg': {
    ...
    def _extract_functions(self, code):
        ...
        functions = []
        # 简单的函数提取正则表达式
        function_pattern = re.compile(r'\s*fn\s+(\w+)\s*\(([^)]*)\)\s*(->\s*[^\{]+)?\s*[{]', re.MULTILINE)
    ...
    def _extract_structs(self, code):
        ...
        structs = []
        # 简单的结构体提取正则表达式
        struct_pattern = re.compile(r'\s*struct\s+(\w+)\s*[{]', re.MULTILINE)
    ...
    def _extract_enums(self, code):
        ...
        enums = []
        # 简单的枚举提取正则表达式
        enum_pattern = re.compile(r'\s*enum\s+(\w+)\s*[{]', re.MULTILINE)
