...
class GoogleProvider(AIProvider):
    """Google Gemini API provider for strategy generation"""
    ...
    def __init__(self, api_key: Optional[str] = None, config: Optional[Dict[str, Any]] = None):
        ...
        self.max_output_tokens = self.config.get('max_output_tokens', 1000)
        self.timeout = self.config.get('timeout', 30)
        
    def generate_strategy(self, code_analysis: Dict[str, Any], performance_mode: str = 'balanced') -> Dict[str, Any]:
        """Generate confusion strategy using Google Gemini API
...
4. Only return the JSON object, no additional text."""
    ...
    def _parse_strategy(self, content: str) -> Dict[str, Any]:
        """Parse strategy from Google Gemini response
...
# Register provider
AIProviderFactory.register_provider('google', GoogleProvider)
