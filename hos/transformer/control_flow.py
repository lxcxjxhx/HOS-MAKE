...
class ControlFlowTransformer:
    ...
    def _flatten_function_body(self, body):
        ...
        # 生成平坦化代码
        flattened = []
        flattened.append(f"{indent}{state_var} = 0")
        flattened.append(f"{indent}while True:")
        flattened.append(f"{indent}    if {state_var} == 0:")
        
        # 添加每个代码块
        for i, block in enumerate(blocks):
            if i > 0:
                flattened.append(f"{indent}    elif {state_var} == {i}:")
            ...
            # 添加状态转换
            if i < len(blocks) - 1:
                flattened.append(f"{indent}        {state_var} = {i + 1}")
            else:
                flattened.append(f"{indent}        break")
